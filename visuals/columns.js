const column = require('column-mesh')
...
module.exports = function ({regl, keyboard}) {
  ...
  const drawColumn = regl({
    frag: `
      precision mediump float;
      varying vec3 vnormal, fposition;
      ...
      void main () {
        float intensity =
          (1.0 + sin(
            3.0 * fposition.z +
            0.5 * fposition.y +
            3.0 * snoise(0.6 * fposition))) / 2.0;
        float light = max(dot(vnormal, vec3(sqrt(2.0), sqrt(2.0), 0) / 2.0), 0.0) + 0.1;
        intensity *= light;
  ...
  vec4 feedback(vec2 uv, float t, vec3 keys[NUM_KEYS], sampler2D image[2]) {
    return 0.8 * vec4(texture2D(image[1], uv).rgb, 0.5);
  }
  `,
    draw: () => {
      regl.clear({
